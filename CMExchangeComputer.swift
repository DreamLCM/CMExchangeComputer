...
class CMExchangeComputer: UIView {
    ...
    //可卖可买数量
    @IBOutlet weak var labelExvhangeAmount: UILabel!
    //币种当前价格
    ...
    ///创建实例
    class func getInstance(showType: String, dismissViewDoWhat: @escaping ButtonDismissDoWhat,exchangeDoWhat: @escaping ButtonExchangeDoWhat) -> CMExchangeComputer {
        ...
        cmExchangeComputer.keyboardHeight = 327
        ...
        cmExchangeComputer.setExchangeComputer(cmExchangeComputer: cmExchangeComputer, showType: showType)
    ...
    /**
     设置自定义键盘样式
     */
    func setExchangeComputer(cmExchangeComputer:CMExchangeComputer,showType: String) {
        self.labelAccount.text = "多币种账户".locals
        if showType == "buy" {
            
            // 买入样式
            for buttonSingle in cmExchangeComputer.keyboardButtons {
                buttonSingle.setTitleColor(ExchangeType.Buy.typeColor(), for: UIControlState.normal)
                
            }
            cmExchangeComputer.button_confirm.backgroundColor = ExchangeType.Buy.typeColor()
            cmExchangeComputer.button_confirm.setTitle("买入".localized(), for: .normal)
            cmExchangeComputer.sliderExchange.minimumTrackTintColor = ExchangeType.Buy.typeColor()
            cmExchangeComputer.sliderExchange.thumbTintColor = ExchangeType.Buy.typeColor()
            cmExchangeComputer.sliderExchange.setThumbImage(UIImage(named:"thumb_buy"), for: .normal)
            cmExchangeComputer.showType = showType
            cmExchangeComputer.button_del.setImage(UIImage(named:"back_buy"), for: .normal)
            cmExchangeComputer.button_dot.setImage(UIImage(named:"point_buy"), for: .normal)
            cmExchangeComputer.switchBorrow.onTintColor = ExchangeType.Buy.typeColor()
            cmExchangeComputer.cmCountTF.placeholder = "买入数量".locals
            cmExchangeComputer.cmPriceTF.placeholder = "买入价格".locals
        } else {
            
            // 卖出样式
            for buttonSingle in cmExchangeComputer.keyboardButtons {
                buttonSingle.setTitleColor(ExchangeType.Sell.typeColor(), for: UIControlState.normal)
            }
            cmExchangeComputer.button_confirm.backgroundColor = ExchangeType.Sell.typeColor()
            cmExchangeComputer.button_confirm.setTitle("卖出".localized(), for: .normal)
            cmExchangeComputer.showType = showType
            cmExchangeComputer.sliderExchange.minimumTrackTintColor = ExchangeType.Sell.typeColor()
            cmExchangeComputer.sliderExchange.thumbTintColor = ExchangeType.Sell.typeColor()
            cmExchangeComputer.sliderExchange.setThumbImage(UIImage(named:"thumb_sell"), for: .normal)
            cmExchangeComputer.button_del.setImage(UIImage(named:"back_sell"), for: .normal)
            cmExchangeComputer.button_dot.setImage(UIImage(named:"point_sell"), for: .normal)
            cmExchangeComputer.switchBorrow.onTintColor = ExchangeType.Buy.typeColor()
            cmExchangeComputer.cmCountTF.placeholder = "卖出数量".locals
            cmExchangeComputer.cmPriceTF.placeholder = "卖出价格".locals
        }
